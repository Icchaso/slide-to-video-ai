...
| 🎬 レイアウト | スライドは画面いっぱい。テロップはスライドに重ねて**画面の下20%の中**に収める（スライドの中身は上80%に置く前提。slidesmith なら deck.json に `"video": true`）。場面ごとの位置は `storyboard.json` で変更可 |
...
| テロップ | 画面の下20%（216px）の中・52px・1行16文字 × 2行以内、白文字＋黒縁＋半透明帯、音声より 80ms 先行して即時表示、改行は読点＞文節境界（「という」「もっと」の途中で切らない） | 放送・映像翻訳業界の基準 |
| 映像 | スライド全面・クロスフェード 0.6秒・16:9 以外はぼかし背景で黒帯なし。ゆっくりズーム（Ken Burns）は既定オフ（`animation.ken_burns: true` で有効。全面スライドの端が切れるため） | 文字スライドが破綻しない |
