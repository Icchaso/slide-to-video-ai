...
| 3.5 | BGM 候補: 「チョイス悪い・軽快ポップで日本人に馴染みのある有名フリー素材を」（いっちゃん 2026-09-17）→ 既定の出どころを **BGMer**（`src/bgm_jp.py`・約550曲の一覧を7日キャッシュ・DL 数で人気順）に。パターン5種（軽快ポップ/おしゃれ・Chill/ほのぼの日常/前向き・感動/スタイリッシュ）から最大8曲、おすすめ3曲のうち2曲は軽快ポップ。採用時は長い版を保存。Openverse は予備。**規約調査**: OpenTracks（旧 DOVA-SYNDROME、2026-09-15 改名）は bot 収集禁止、甘茶は直リンク禁止、MusMus は広告ブロック中 DL 不可、魔王魂は mp3 直接取得が 403 → いずれも自動取得せず、候補一覧の末尾にリンクだけ出す | 完了（下のコミット） |
| 3.6 | 通し確認と仕上げ: BGMer の曲（再会の誓い・検証用に仮採用）で test-project を本番書き出し PASS・イントロ→スライドの切替でタイトルが二重に見える件を修正（contrast 警告0）・slidesmith に `"video": true`（下20%を空けて描画、帯に入れば `VIDEO_BAND` で停止。slidesmith 2833627） | 完了 291f18b |
| 4 | README・乗り換え手順書の BGM 記述更新・判断記録・メモリ更新・共有相手への切り替え案内文 | 完了（下のコミット） |
...
**残課題**: なし。**いっちゃん本人はこのツールを使わない（2026-09-17）**。使うのは共有相手で、Fish Audio のキーは相手が自分で取る（手順書のトラブル表に記載）。test-project の BGM 1番・`.env` の失効キーは検証用のまま放置でよい。
